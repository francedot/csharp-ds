...
namespace CSharp.DS.Hash
{
     ...
     */
    /// <summary>
    /// Hashset implementation - Separate Chaining of Bucket using LinkedLists
    /// </summary>
    public class HashSet_LinkedList
    {
        // Time: O(N/K) where N is the number of all possible values and K is the number of predefined buckets, which is 431.
        ...
        {
            private readonly LinkedList<int> _elements;

            public Bucket()
            {
                _elements = new LinkedList<int>();
            }

            public void Add(int key)
            {
                if (!Any(key))
                    _elements.AddFirst(key);
            }
            ...
            public bool Any(int key) => _elements.Any(e => e.Equals(key));
